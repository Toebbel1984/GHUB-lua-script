...
-- M2 + G2 = Center Toggle
-- M2 + G3 = Spiral Toggle
-- M2 + G4 = Infinity Toggle
-- M2 + G5 = Circle Toggle
-- M3 + G1 = Ellipse Toggle
-- M3 + G2 = Diamond Toggle
-- M3 + G3 = Heart Toggle
-- M3 + G4 = Lissajous Toggle
-- =========================================================
...
local function GetActiveModeName()
    ...
    if EllipseEnabled == 1 then return "Ellipse" end
    if DiamondEnabled == 1 then return "Diamond" end
    if HeartEnabled == 1 then return "Heart" end
    if LissajousEnabled == 1 then return "Lissajous" end
    return "None"
end
...
local function LogStatus(event, arg)
    ...
    if GetMKeyState() == 1 then return end
    ...
    BoxLine("Ellipse     : " .. BoolText(EllipseEnabled))
    BoxLine("Diamond     : " .. BoolText(DiamondEnabled))
    BoxLine("Heart       : " .. BoolText(HeartEnabled))
    BoxLine("Lissajous   : " .. BoolText(LissajousEnabled))
    OutputLogMessage("\t\t┣" .. string.rep("━", BoxWidth + 6) .. "┫\n")
    BoxLine("Cursor      : " .. x .. ", " .. y)
    OutputLogMessage("\t\t┗" .. string.rep("━", BoxWidth + 6) .. "┛\n")
end
...
local function RunParametricMouse(step, sleepMs, pointFunction)
    local t = 0
    local lastX, lastY = pointFunction(t)
    local fracX = 0
    local fracY = 0
    while IsMouseButtonPressed(1) do
        ...
        local curX, curY = pointFunction(t)
...
function EllipseMouse()
    RunParametricMouse(EllipseStep, EllipseSleep, function(t)
        return math.cos(t) * EllipseRadiusX,
           math.sin(t) * EllipseRadiusY
    end)
end

function DiamondMouse()
    RunParametricMouse(DiamondStep, DiamondSleep, function(t)
        local sinT = math.sin(t)
        local cosT = math.cos(t)
        local divisor = math.abs(cosT) + math.abs(sinT)
        return DiamondSize * cosT / divisor,
           DiamondSize * sinT / divisor
    end)
end

function HeartMouse()
    RunParametricMouse(HeartStep, HeartSleep, function(t)
        local sinT = math.sin(t)
        local x = 16 * sinT * sinT * sinT
        local y = 13 * math.cos(t)
            - 5 * math.cos(2 * t)
            - 2 * math.cos(3 * t)
            - math.cos(4 * t)
        return x * HeartScale, -y * HeartScale
    end)
end

function LissajousMouse()
    RunParametricMouse(LissajousStep, LissajousSleep, function(t)
        return LissajousRadiusX * math.sin(LissajousFrequencyX * t + LissajousPhase),
           LissajousRadiusY * math.sin(LissajousFrequencyY * t)
    end)
end
...
local function ToggleEllipse()
    local wasEnabled = EllipseEnabled
    if ExclusiveModes then DisableAllModes() end
    EllipseEnabled = (wasEnabled == 1) and 0 or 1
    Announce("Ellipse = " .. BoolText(EllipseEnabled))
end

local function ToggleDiamond()
    local wasEnabled = DiamondEnabled
    if ExclusiveModes then DisableAllModes() end
    DiamondEnabled = (wasEnabled == 1) and 0 or 1
    Announce("Diamond = " .. BoolText(DiamondEnabled))
end

local function ToggleHeart()
    local wasEnabled = HeartEnabled
    if ExclusiveModes then DisableAllModes() end
    HeartEnabled = (wasEnabled == 1) and 0 or 1
    Announce("Heart = " .. BoolText(HeartEnabled))
end

local function ToggleLissajous()
    local wasEnabled = LissajousEnabled
    if ExclusiveModes then DisableAllModes() end
    LissajousEnabled = (wasEnabled == 1) and 0 or 1
    Announce("Lissajous = " .. BoolText(LissajousEnabled))
end
...
function OnEvent(event, arg)
    ...
    if event == "G_PRESSED" and current_mkey == 2 then
        if arg == 1 then
            ToggleRecoil()
        elseif arg == 2 then
            ToggleCenter()
    ...
    if event == "MOUSE_BUTTON_PRESSED" and arg == 1 and current_mkey == 3 then
        if EllipseEnabled == 1 then
            EllipseMouse()
        elseif DiamondEnabled == 1 then
            DiamondMouse()
        elseif HeartEnabled == 1 then
            HeartMouse()
        elseif LissajousEnabled == 1 then
            LissajousMouse()
        end
    end
    if event == "MOUSE_BUTTON_PRESSED" and arg == 1 and current_mkey == 2 then
